...
namespace Orchard.CRM.Core.Providers.Filters
{
    public class ContentItemPermissionFilter : IFilterProvider
    {
        ...
        public ContentItemPermissionFilter(IOrchardServices orchardServices, IBasicDataService basicDataService)
        {
        ...
        public void Describe(DescribeFilterContext describe)
        {
            ...
            // Team
            descriptor.Element(
                type: "ContentItemPermissionPartRecord.Team_Id",
                name: new LocalizedString("Team"),
                description: T("Team Selection"),
                filter: (context) => ApplyTeamFilter(context),
                display: context => Display(context, "Team_Id"),
                form: TeamSelectionForm.FormName
                    );
            ...
            descriptor.Element(
                ...
                display: context => Display(context, "BusinessUnit_Id"),
            ...
            descriptor.Element(
                ...
                display: context => Display(context, "ItemsWithoutAnyAsignee")
                    );
        }
        ...
        public void ApplyTeamFilter(FilterContext context)
        {
            if (context.State.Team_Id == null || string.IsNullOrEmpty(context.State.Team_Id.Value))
            {
                return;
            }

            int value = context.State.Team_Id;
            this.ApplyFilter(context, "Team", "ContentItemPermissionDetailRecords_TeamPart", value);
        }
    ...
    public class TeamSelectionForm : BasicDataFilterForm
    {
        private IRepository<TeamPartRecord> teamRepository;

        public const string FormName = "TeamSelectionForm";
        public TeamSelectionForm(IShapeFactory shapeFactory, IRepository<TeamPartRecord> teamRepository)
            : base(shapeFactory)
        {
            this.teamRepository = teamRepository;
            this.formName = FormName;
            this.selectName = "Team_Id";
            this.selectId = "Team_Id";
            this.selectTitle = "Team ID";
            this.selectDescription = "Team Selection";
            this.selectSize = 20;
        }

        protected override IList<SelectListItem> GetData()
        {
            var items = teamRepository.Table.OrderBy(c => c.Name).ToList().Select(c => new SelectListItem
            {
