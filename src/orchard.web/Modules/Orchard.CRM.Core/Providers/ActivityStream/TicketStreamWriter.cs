...
namespace Orchard.CRM.Core.Providers.ActivityStream
{
    ...
    public class TicketStreamWriter : IActivityStreamWriter
    {
        ...
        public KeyValuePair<string, int> GetContentDescription(ActiviyStreamWriterContext context)
        {
            ...
            {
                return new KeyValuePair<string, int>(this.GetCreatedTicketDescription(ticketPart), 1);
            }

            string ticketDescription = this.GetTicketIdAndTitle(ticketPart.Record);

            return new KeyValuePair<string, int>(string.Format("{0} {1}", this.T("on Ticket").Text, ticketDescription), 1);
        }
