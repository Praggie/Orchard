...
namespace Orchard.CRM.Core.Controllers
{
    ...
    public class CRMCommentController : Controller, IUpdateModel
    {
        ...
        public ActionResult Add(int contentId, string comment, string returnUrl)
        {
            ...
            string contentDescription = string.Format("{0} {1} - {2}", T("Ticket").Text, identityString, title);
            this.activityStreamService.Add(userId, contentItem.Id, contentItem.VersionRecord.Id, new[] { "commented on the Ticket" }, contentDescription);
