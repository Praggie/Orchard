...
namespace Orchard.CRM.Core.Controllers
{
    ...
    public class TicketController : BaseController
    {
        ...
        public TicketController(
            ...
            : base("Ticket", "Ticket_Edit", indexProvider, services, crmContentOwnershipService, transactionManager, extendedContentManager, contentManager, partSerializationManager, widgetService, themeAwareViewEngine, shapeFactory, displayHelperFactory, basicDataService, contentOwnershipHelper, streamService)
        {
        ...
        public ActionResult CreateChildTicket(int parentTicketId)
        {
            ...
            var parentTicketContentItem = this.contentManager.Get(parentTicketId);
            ...
            return this.CreateActionResultBasedOnAjaxRequest(model, "Create", "Create");
        }
        ...
        public ActionResult QuickUpdate(TicketQuickUpdateViewModel model)
        {
            ...
            var tickets = this.contentManager.GetMany<TicketPart>(model.Ids, VersionOptions.DraftRequired, new QueryHints()).ToList();
            ...
            if (ModelState.IsValid)
            {
                foreach (var ticket in tickets)
                {
                    ...
                    if (model.UpdateBusinessUnitId || model.UpdateTeamId || model.UpdateUserId)
                    {
                        ...
                        this.EditOwner(contentItem, postedEditOwnerViewModel, false);
            ...
            if (isAjaxRequest)
            {
                var data = new
                {
                    Ticket = this.partSerializationManager.Convert(tickets[0].ContentItem),
        ...
        protected override void OnCreatingPost(ContentItem contentItem)
        {
            ...
            base.OnCreatingPost(contentItem);
        }
