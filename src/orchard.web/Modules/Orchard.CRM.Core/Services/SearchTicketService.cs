...
namespace Orchard.CRM.Core.Services
{
    public class SearchTicketService : ISearchTicketService
    {
        ...
        public IContent[] SearchByDatabase(PagerParametersWithSortFields pagerParameters, PostedTicketSearchViewModel searchModel)
        {
            ...
            var contentItems = contentQuery.Slice((pager.Page > 0 ? pager.Page - 1 : 0) * pager.PageSize, pager.PageSize);
        ...
        private ISearchBuilder CreateLuceneBuilder(PostedTicketSearchViewModel searchModel)
        {
            ...
            if (!String.IsNullOrWhiteSpace(searchModel.Term))
            {
                ...
                {
                    string query = string.Format(CultureInfo.InvariantCulture, "{0}:\"{1}*\" OR {2}:\"{1}*\" OR {3}:\"{1}*\" OR {4}:{1}", TicketPart.DescriptionFieldName, searchText, TicketPart.TitleFieldName, TicketPart.CommentsFieldName, TicketPart.IdentityFieldName);
                    queries.Add(query);
            ...
            {
                string permissionsQuery = this.CreatePermissionQuery(searchModel);
        ...
        public IHqlQuery CreateQuery(PostedTicketSearchViewModel searchModel)
        {
            ...
            var selectedBusinessUnits = searchModel.BusinessUnits.ToList();

            // TeamIds of the search
            var teams = new List<int>();

            // BusinessUnitIds of the search
            List<int> businessUnits = selectedBusinessUnits.ToList();

            // Ids of the searched users
            List<int> users = searchModel.Users.ToList();
            ...
            else
            {
                ...
                if (!this.services.Authorizer.Authorize(Permissions.AdvancedOperatorPermission))
                {
                    ...
                    this.RestrictLists(ref teams, ref users, ref businessUnits, out allItemsUserCanSee, searchModel);
        ...
        private string CreatePermissionQuery(PostedTicketSearchViewModel searchModel)
        {
            // TeamIds of the search
            var teams = new List<int>();

            // BusinessUnitIds of the search
            List<int> businessUnits = searchModel.BusinessUnits.ToList();
        ...
        private void RestrictLists(ref List<int> teams, ref List<int> users, ref List<int> businessUnits, out bool allItemsUserCanSee, PostedTicketSearchViewModel searchModel)
        {
