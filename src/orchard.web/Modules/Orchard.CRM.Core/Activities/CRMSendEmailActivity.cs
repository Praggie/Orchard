...
namespace Orchard.CRM.Core.Activities
{
    public abstract class CRMSendEmailActivity : Task
    {
        ...
        protected void SendEmail(string subject, string body, string recipients, bool queued, int priority)
        {
            ...
            if (!queued)
            {
                this.messageService.Send(SmtpMessageChannel.MessageType, parameters);
            }
            else
            {
                this.jobsQueueService.Enqueue("IMessageService.Send", new { type = SmtpMessageChannel.MessageType, parameters = parameters }, priority);
            }
