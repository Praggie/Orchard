        ...
        private void ProcessServerData(FirebaseServerEventType serverEvent, string serverData)
        {
            switch (serverEvent)
            {
                ...
                case FirebaseServerEventType.Patch:
                    ...
                    {
                       // var obj = this.cache.GetNewObject(path, data);

                        //var item = JsonConvert.DeserializeObject<T>(data);

                        var obj = new FirebaseObject<T>(path, (T)msg);

                        this.observer.OnNext(new FirebaseEvent<T>(obj.Key, obj.Object, eventType));
                    }
                    else
                    {
