...
namespace Firebase.Database.Streaming
{
    ...
    public class FirebaseCache<T> : IEnumerable<FirebaseObject<T>>
    {
        ...
        public FirebaseObject<T> GetNewObject(string path, string data) 
        {
            ...
            return new FirebaseObject<T>(path, (T)item);
        }
        ...
        public IEnumerable<FirebaseObject<T>> PushData(string path, string data) 
        {
            ...
            foreach (var element in pathElements)
            {
                ...
                else
                {
                    ...
                    var objParent = obj;
                         JToken myTest = ((JObject)objParent).Descendants()
                        .Where(t => t.Type == JTokenType.Property && ((JProperty)t).Name == element)
                        .Select(p => ((JProperty)p).Value)
                        .FirstOrDefault();
                    if (myTest != null) //i.E new item is being added.
                    {

                        var valueType = myTest.GetType();
                        if (valueType == typeof (JValue))
                        {
                            obj = (JToken) (myTest.Parent);
                        ...
                        {
                            objParent = obj= myTest;

                        }

                        objDeleter = () => ((JProperty) objParent).Value = null;
                        primitiveObjSetter = (d) => ((JProperty) objParent).Value = new JValue(d);

                        ///
                        /// 
                        /// 
                    /*    var property = ((dynamic) objParent).Property(element);
                        if (property != null)
                        {
                            objDeleter = () => property.Value = null;
                            primitiveObjSetter = (d) => property.Value = d;
                            obj = property.Value;
                    ...
                    else
                    {
                      ...
                      var  property = objParent
                            ...
                            .First(
                                p =>
                                    p.Name.Equals(element, StringComparison.OrdinalIgnoreCase) ||
            ...
            else
            {
                ...
                if (valueType == typeof (JProperty))
                {
                    primitiveObjSetter(data);
                }
                else
                {
                    ...
                    if ((valueType.GetTypeInfo().IsPrimitive || valueType == typeof (JObject)) &&
